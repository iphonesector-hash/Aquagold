...
# The newer lightweight Today Tour was added after the mature Smart Tour. Reuse
# the mature source loader so Preview receives the same live new/review Bale rows
# (including its preview read-only sync and schedule parser) instead of querying an
# isolated preview table before it has been synchronized.
aqua_today_tour._active_rows = aqua_smart_tour._active_today_rows
...
def _decorate_result(item, index, query, source):
    """Return one address result in the shape Aqua's generic result UI can show."""
...
def _address_results(query, limit=3):
    """Prefer Neshan for Iranian/Persian addresses, fall back to existing provider."""
    try:
...
def aria_map_chat():
    ...
    # Neshan Geocoding Plus often returns a precise first result plus broad
    # alternatives. If the query is specific and the first result has no
    # unmatched term, open it directly; otherwise present selectable results.
    first = results[0]
    precise_first = (
    ...
    return jsonify({
        ...
        "query": query,
        "results": results,
        "action": {"type": "show_address_on_map", "location": selected},
    })
...
function mapLayout(){const c=$('#aqst-controls'),d=$('#aqst-tour-details');if(c&&d){d.parentNode.insertBefore(c,d);c.classList.add('aq-controls-below')}$('#aquaTodayTourLaunch')?.remove()}
...
def inject_aria_map(response):
    if request.path in {"/", "/index.html"} and response.mimetype == "text/html":
        ...
        if "/aqua-final-task-fix.js" not in body:
            body = body.replace("</body>", '<script src="/aqua-final-task-fix.js?v=20260914-1"></script></body>', 1)
