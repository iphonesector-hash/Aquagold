...
(()=>{
 ...
 window.app=function(){
  ...
  s.toggleAquaRecording=async function(){
   ...
   this.stopAquaSpeech();this.primeAquaDeviceSpeech();this.setAquaVoicePhase('starting');this.toast?.('در حال درخواست میکروفن…','info');let stream=null;
   try{
...
def inject_aqua_voice_controller(response):
    try:
        if request.path in {"/", "/index.html"} and response.mimetype == "text/html":
            ...
            if position >= 0:
                tag = '<script src="/aqua-voice-ui.js?v=20260903-aria2"></script>'
