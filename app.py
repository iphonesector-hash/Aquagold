...
import finance_private_report  # noqa: E402,F401
...
def _health_with_ai_status():
    ...
    if response.is_json:
        payload = response.get_json() or {}; status = aqua_ai.configuration_status()
        payload["ai"] = "configured" if status["brain"] else "not_configured"; payload["aqua_ai"] = status
        try:
