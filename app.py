...
def _row_json(row):
    ...
    for key,value in dict(row).items():
        ...
        elif isinstance(value,UUID):value=str(value)
        elif isinstance(value,(date,datetime)):value=value.isoformat()
        out[key]=value
    return out
...
# Today Tour compatibility must load after Smart Tour because it reuses its synced source.
...
import aqua_numeric_digits  # noqa: E402,F401
...
# Last scoped CSS layer fixes only requested expense/map layout; no AI provider/mic changes.
...
def _health_with_ai_status():
    ...
    if response.is_json:
        payload=response.get_json() or {};status=aqua_ai.configuration_status();payload['ai']='configured' if status['brain'] else 'not_configured';payload['aqua_ai']=status;payload['neshan']=aqua_neshan_preview.configuration_status()
        try:
            bale=bale_bridge._public_settings(bale_bridge._load_settings());payload['bale']={'enabled':bale['enabled'],'token':bale['bot_token_configured'],'webhook':bale['webhook_configured']}
        except Exception:payload['bale']={'enabled':False,'token':False,'webhook':False}
