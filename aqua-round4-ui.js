(()=>{
  ...
  const paymentLabel=value=>({cash:'نقد',transfer:'کارت به کارت',card:'کارتخوان'})[value]||value||'—';

  function normalizeChoices(state){
    const p=state?.smartParsed;if(!p)return;
  ...
  function mountSmartControls(state){
    ...
    if(!card)return;
    const inner=[...card.querySelectorAll('div')].find(el=>el.classList.contains('mt-4')&&el.querySelector('.gps-ready,.glass'))||card.querySelector('template')?.nextElementSibling;
    const gps=[...card.querySelectorAll('div')].find(el=>el.querySelector?.('[x-text*="GPS"]')||el.getAttribute?.(':class')?.includes('smartGps'));
    let controls=card.querySelector('#aqua-smart-choice-controls');
    if(!controls){
  ...
  function mountLatestServicesAccordion(){
    ...
    const card=title?.closest('.card');if(!card||card.dataset.aquaRound4Accordion)return;
    ...
    toggle.type='button';toggle.className='btn soft !py-2 !px-3 mt-2';toggle.textContent='نمایش آخرین سرویس‌ها';
    titleBox.appendChild(toggle);
    const setClosed=closed=>{card.classList.toggle('aqua-latest-collapsed',closed);toggle.textContent=closed?'نمایش آخرین سرویس‌ها':'بستن آخرین سرویس‌ها';toggle.setAttribute('aria-expanded',String(!closed))};
    toggle.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();setClosed(!card.classList.contains('aqua-latest-collapsed'))});
    setClosed(true);
  }
  ...
  function mountFinancePaymentUi(){
    ...
    const grid=monthly?.closest('.grid');if(!grid)return;
    ...
    if(!document.getElementById('paymentMethodChart')){
      const card=document.createElement('div');card.className='card p-5 aqua-payment-chart-card';card.innerHTML='<h3 class="font-black mb-3">روش‌های پرداخت</h3><div class="aqua-payment-chart-wrap"><canvas id="paymentMethodChart"></canvas></div>';
  ...
  window.app=function(){
    ...
    state.analyzeSmart=async function(...args){
      const result=await oldAnalyze?.(...args);normalizeChoices(this);
      setTimeout(()=>mountSmartControls(this),30);setTimeout(()=>mountSmartControls(this),180);
      return result;
    };
    ...
    state.renderRound4Finance=async function(){
      ...
      for(const key of ['cash','transfer','card']){const el=document.querySelector(`[data-aqua-payment-total="${key}"]`);if(el)el.textContent=this.money(Number(totals[key]||0))+' تومان'}
      const destroy=name=>{try{this[name]?.destroy?.()}catch{}this[name]=null};
      const monthly=(this.jalaliMonthlyMetrics||[]).slice(-12),yearly=(this.jalaliYearlyMetrics||[]).slice(-6);
      const services={};for(const job of (this.jobs||[])){const name=clean(job.service_type)||'دیگر';services[name]=services[name]||{name,received:0};services[name].received+=Number(job.received_amount||0)}
      ...
      const monthlyCanvas=document.getElementById('monthlyChart');if(monthlyCanvas){destroy('monthlyChart');this.monthlyChart=new Chart(monthlyCanvas,{type:'line',data:{labels:monthly.map(x=>x.label),datasets:[{label:'دریافتی',data:monthly.map(x=>x.received),borderColor:'#22c55e',backgroundColor:'rgba(34,197,94,.12)',tension:.3,fill:true},{label:'سهم شرکت',data:monthly.map(x=>x.company_share),borderColor:'#06b6d4',tension:.3},{label:'هزینه',data:monthly.map(x=>x.expenses),borderColor:'#f59e0b',tension:.3},{label:'سود خالص',data:monthly.map(x=>x.net_profit),borderColor:'#8b5cf6',tension:.3}]},options:{...common,scales:{y:{beginAtZero:true}}}})}
      const yearlyCanvas=document.getElementById('yearlyChart');if(yearlyCanvas){destroy('yearlyChart');this.yearlyChart=new Chart(yearlyCanvas,{type:'bar',data:{labels:yearly.map(x=>x.year),datasets:[{label:'دریافتی',data:yearly.map(x=>x.received),backgroundColor:'rgba(34,197,94,.72)'},{label:'سود خالص',data:yearly.map(x=>x.net_profit),backgroundColor:'rgba(139,92,246,.72)'}]},options:{...common,scales:{y:{beginAtZero:true}}}})}
      const serviceCanvas=document.getElementById('serviceChart');if(serviceCanvas){destroy('serviceChart');this.serviceChart=new Chart(serviceCanvas,{type:'bar',data:{labels:serviceRows.map(x=>x.name),datasets:[{label:'دریافتی',data:serviceRows.map(x=>x.received),backgroundColor:'rgba(14,165,168,.72)'}]},options:{...common,indexAxis:'y',scales:{x:{beginAtZero:true}}}})}
      const payment=document.getElementById('paymentMethodChart');if(payment){destroy('financeDonutChart');this.financeDonutChart=new Chart(payment,{type:'doughnut',data:{labels:['نقد','کارت به کارت','کارتخوان'],datasets:[{data:[Number(totals.cash||0),Number(totals.transfer||0),Number(totals.card||0)],backgroundColor:['#22c55e','#3b82f6','#8b5cf6'],borderWidth:0}]},options:{...common,cutout:'62%',plugins:{...common.plugins,tooltip:{callbacks:{label:ctx=>`${paymentLabel(['cash','transfer','card'][ctx.dataIndex])}: ${this.money(ctx.raw)} تومان`}}}}}})}
    };
  ...
  const mount=()=>{
    if(!document.getElementById('aqua-round4-style')){
      const style=document.createElement('style');style.id='aqua-round4-style';style.textContent=`
