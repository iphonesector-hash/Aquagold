function app(){return{
  ...
  navs:[
    {id:'dashboard',label:'داشبورد',icon:'home'},{id:'daily',label:'روزانه',icon:'daily'},{id:'customers',label:'مشتریان',icon:'customers'},
    {id:'services',label:'سرویس‌ها',icon:'services'},{id:'map',label:'نقشه',icon:'map'},{id:'expense',label:'هزینه‌ها',icon:'expenses'},
    {id:'finance',label:'مالی',icon:'finance'},{id:'insights',label:'تحلیل',icon:'insights'},{id:'smart',label:'هوشمند',icon:'smart'},
    {id:'reminders',label:'یادآوری',icon:'reminders'},{id:'settings',label:'تنظیمات',icon:'settings'}
  ],
  ...
  setAuthViewState(authenticated){if(document.body)document.body.dataset.authState=authenticated?'authenticated':'guest'},
  ...
  async api(path,opts={}){
    ...
    try{
      ...
      if(r.status===401&&!['/login','/session','/logout'].includes(path)){this.token=false;this.user=null;this.setAuthViewState(false);let e=Error('نشست منقضی شد');e.status=401;throw e}
  ...
  async init(){
    ...
    try{
      ...
      window.addEventListener('online',async()=>{this.online=true;if(await this.finishPendingLogout()){this.token=false;this.user=null;this.authReady=true;return}this.syncOfflineQueue()});
      ...
      try{let d=await this.api('/session');if(!d?.user||(d.expires_at&&Date.parse(d.expires_at)<=Date.now()))throw Error('نشست آفلاین منقضی شده');await this.bindOfflineUser(d.user);if(window.AquaOffline)await AquaOffline.cachePut('/api/session',d);this.token=true;this.user=d.user;this.setAuthViewState(true);await this.refreshAll();this.locate(false).catch(()=>{});await this.syncOfflineQueue()}catch{this.token=false;this.user=null;this.setAuthViewState(false)}
    }catch(error){this.token=false;this.user=null;this.setAuthViewState(false);console.warn('AquaGold initialization fallback',error)}finally{this.authReady=true}
  },
  async login(){this.busy=true;this.error='';try{let d=await this.api('/login',{method:'POST',body:JSON.stringify(this.loginForm)});try{localStorage.removeItem('aq_logout_pending')}catch{}await this.bindOfflineUser(d.user);if(window.AquaOffline)await AquaOffline.cachePut('/api/session',{authenticated:true,user:d.user,expires_at:d.expires_at});this.token=true;this.user=d.user;this.setAuthViewState(true);await this.refreshAll()}catch(e){this.setAuthViewState(false);this.error=e.status===429?'تلاش‌های ورود بیش از حد است؛ کمی بعد دوباره امتحان کن':!navigator.onLine||!e.status?'برای ورود اولیه اتصال اینترنت لازم است':'نام کاربری یا رمز عبور نادرست است'}finally{this.busy=false;this.authReady=true}},
  async logout(){if(this.busy)return;this.busy=true;let revoked=!this.token;try{if(this.token){await this.api('/logout',{method:'POST'});revoked=true}}catch{}if(window.AquaOffline)await AquaOffline.clear();try{if(revoked)localStorage.removeItem('aq_logout_pending');else localStorage.setItem('aq_logout_pending','1');localStorage.removeItem('aq_offline_user');localStorage.removeItem('aq_drafts_v4')}catch{}this.token=false;this.user=null;this.setAuthViewState(false);this.offlineQueueCount=0;this.authReady=true;sessionStorage.removeItem('aq_unlocked');this.busy=false},
  async refreshAll(){
  ...
  renderMainMap(){
    ...
    for(let c of this.customers.filter(x=>x.latitude&&x.longitude)){let label=c.map_label||c.name,phone=(c.phones||[])[0]||'',route=`https://www.google.com/maps/dir/?api=1&destination=${encodeURIComponent(c.latitude+','+c.longitude)}`,html=`<div dir="rtl" class="aq-map-popup"><b>${this.escapeHtml(label)}</b><div>${this.escapeHtml(phone)}</div><div>${this.escapeHtml(c.address||'')}</div><div>آخرین سرویس: ${this.escapeHtml(c.last_service||'—')}</div><div>آخرین مبلغ: ${this.money(c.last_amount||0)} تومان</div><div class="flex gap-2 mt-2"><button type="button" data-open-customer="${this.escapeHtml(c.id)}" class="btn soft !py-2">پرونده مشتری</button><a href="${route}" target="_blank" rel="noopener" class="btn primary !py-2">مسیریابی</a></div></div>`,m=L.marker([c.latitude,c.longitude],{icon:this.mapIcon()}).bindPopup(html).addTo(this.mainMap);m.on('popupopen',()=>{document.querySelector(`[data-open-customer="${CSS.escape(String(c.id))}"]`)?.addEventListener('click',()=>this.openCustomer(c),{once:true})});this.mainMarkers.push(m);pts.push([c.latitude,c.longitude])}
  ...
  async createExpense(){if(this.busy)return;this.busy=true;try{let f={...this.expenseForm,amount:this.num(this.expenseForm.amount)},d=await this.api('/expenses',{method:'POST',body:JSON.stringify(f)});this.expenseForm={category:'goods',title:'',amount:'',expense_date:'',notes:''};if(d?.queued){this.expenses.unshift({...f,id:d.id,expense_date:f.expense_date||new Date().toISOString(),offline_pending:true});alert('هزینه در صف همگام‌سازی ذخیره شد');return}await this.refreshAll();alert('هزینه ثبت شد')}catch(e){alert(e.message)}finally{this.busy=false}},
  ...
  async createSettlement(){if(this.busy)return;this.busy=true;try{let f={...this.settlementForm,amount:this.num(this.settlementForm.amount)},d=await this.api('/settlements',{method:'POST',body:JSON.stringify(f)});this.settlementForm={amount:'',settled_at:'',notes:''};if(d?.queued){this.settlements.unshift({...f,id:d.id,settled_at:f.settled_at||new Date().toISOString(),offline_pending:true});alert('تسویه در صف همگام‌سازی ذخیره شد');return}await this.refreshAll();this.renderCharts();alert('تسویه ثبت شد')}catch(e){alert(e.message)}finally{this.busy=false}},
  ...
  async registerSmart(){if(this.busy)return null;this.busy=true;try{let p=this.smartParsed||{},d=await this.api('/smart/register',{method:'POST',body:JSON.stringify({text:this.smartText,parsed:p,customer_id:this.smartCustomerId||null,latitude:this.smartGps.lat,longitude:this.smartGps.lng,accuracy:this.smartGps.accuracy,visited_at:p.visited_at||null,scheduled_until:p.scheduled_until||null,invoice_amount:p.amount||0,received_amount:p.amount||0,description:p.description||''})});this.smartText='';this.smartParsed=null;this.smartSuggestions=[];this.smartCustomerId='';if(d?.queued){alert('ثبت هوشمند در صف همگام‌سازی ذخیره شد');this.page='dashboard';return d}await this.refreshAll();alert('ثبت هوشمند انجام شد');let c=this.customers.find(x=>String(x.id)===String(d.customer_id));if(c)this.openCustomer(c);return d}catch(e){alert(e.message);return null}finally{this.busy=false}}
}}
