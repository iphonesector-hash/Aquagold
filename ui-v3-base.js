function app(){return{
  ...
  async login(){this.busy=true;this.error='';try{let d=await this.api('/login',{method:'POST',body:JSON.stringify(this.loginForm)});try{localStorage.removeItem('aq_logout_pending')}catch{}await this.bindOfflineUser(d.user);if(window.AquaOffline)await AquaOffline.cachePut('/api/session',{authenticated:true,user:d.user,expires_at:d.expires_at});this.token=true;this.user=d.user;this.loginForm={username:'',password:''};await this.refreshAll()}catch(e){this.error=e.status===429?'تلاش‌های ورود بیش از حد است؛ کمی بعد دوباره امتحان کن':!navigator.onLine||!e.status?'برای ورود اولیه اتصال اینترنت لازم است':'نام کاربری یا رمز عبور نادرست است'}finally{this.busy=false;this.authReady=true}},
  ...
  async go(p){this.page=p;if(['dashboard','daily','customers','services','expense','finance','insights','reminders','settings'].includes(p))await this.refreshAll();if(p==='map')setTimeout(()=>this.renderMainMap(),100);if(p==='finance'){setTimeout(()=>this.renderCharts(),120);this.guardSettlementAmount();[50,250,700,1600].forEach(ms=>setTimeout(()=>this.guardSettlementAmount(),ms))}},
