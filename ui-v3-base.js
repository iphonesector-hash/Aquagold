function app(){return{
  ...
  async refreshAll(){
    ...
    let [s,j,c,e,set,a,r,fs,i,au]=await Promise.all([
      this.api('/stats'),this.api(`/jobs?page=${this.jobPagination.page}&per_page=${this.jobPagination.per_page}&q=${jq}`),this.api(`/customers?page=${this.customerPagination.page}&per_page=${this.customerPagination.per_page}&q=${cq}`),this.api('/expenses'),this.api('/settlements'),this.api('/reports/analytics'),this.api('/reminders?days=45'),this.api('/settings/finance'),this.api('/reports/insights'),this.canAdmin?this.api('/audit?limit=30'):Promise.resolve([])
    ]);
