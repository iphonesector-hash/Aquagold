...
function aqMatchedPosition(pos,route){const p=aqRouteProjection(pos,route);return p.d<=120?{lat:p.lat,lng:p.lng,_route_i:p.i,_snap_d:p.d}:{...pos,_route_i:p.i,_snap_d:p.d}}
function aqRemainingRoutePoints(route,projection){const pts=route?.points||[];if(!pts.length)return[];const p=projection||{i:0,lat:pts[0][0],lng:pts[0][1]},i=Math.max(0,Math.min(Math.floor(Number(p.i||0)),pts.length-1)),first=[Number(p.lat),Number(p.lng)],rest=pts.slice(i+1);return rest.length?[first,...rest]:[first,first]}
function aqSyncRemainingRoute(route,projection){const points=aqRemainingRoutePoints(route,projection);if(!points.length)return;if(ST.nav.mapKind==='gl'){try{ST.nav.map?.getSource?.('aqua-route')?.setData?.({type:'Feature',properties:{},geometry:{type:'LineString',coordinates:points.map(p=>[p[1],p[0]])}})}catch{}}else{try{ST.nav.casing?.setLatLngs?.(points);ST.nav.line?.setLatLngs?.(points)}catch{}}}
...
#aqst-nav .aqst-gl-user .aqst-user-arrow{width:48px!important;height:52px!important;filter:drop-shadow(0 8px 8px rgba(24,8,54,.48))!important}
