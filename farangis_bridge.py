...
def farangis_customer_payment():
    ...
    with app_v3.get_db() as db, db.cursor() as cur:
        ...
        cur.execute(
            "insert into audit_log(entity_type,entity_id,action,after_data) values('service_visit',%s,'farangis_create',%s::jsonb)",
            (str(visit_id), app_v3.Jsonb({"customer": customer_name, "amount": amount, "source": "farangis"})),
        )
...
def farangis_customer_history():
    ...
    return jsonify({
        ...
        "customer": {
            ...
            "phone": customer.get("phone"),
            "address": customer.get("address"),
            "deviceModel": customer.get("device_model"),
        },
        "latest": latest,
        "visits": visits,
    })
